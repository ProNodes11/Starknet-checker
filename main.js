...
async function fetchInfo(address, unicAddress) {
    try {
        ...
        addRowToCSV(address, nonce, unicAddress.length + 1, await formatDate(firstDay), await formatDate(lastDay),  await dateDiff(firstDay, lastDay))
    } catch (error) {
...
function addRowToCSV(address, nonce, interactedAddress, firstDay, lastDay, dayDiff) {
    const row = `${address},${nonce},${interactedAddress},${firstDay},${lastDay},${dayDiff}\n`;
...
async function main() {
    addRowToCSV('Address', 'TxCount', "Interacted address", "First Day", "Last Day", "Days count")
    let counter = 1
